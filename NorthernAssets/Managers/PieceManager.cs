...
public class BuildPiece
{
    ...
    private static class Piece_Awake_Patch
    {
        ...
        {
            if (__instance.m_placeEffect.m_effectPrefabs.Length > 0) return;
            if (!buildPieceMap.TryGetValue(Helpers.GetNormalizedName(__instance.name), out BuildPiece piece)) return;
            UpdateEffectList(piece.PlaceEffects, ref __instance.m_placeEffect);
            if (__instance.TryGetComponent(out RandomSpeak randomSpeak)) UpdateEffectList(piece.RandomSpeakEffects, ref randomSpeak.m_speakEffects);
            if (__instance.TryGetComponent(out WearNTear wearNTear))
            {
                UpdateEffectList(piece.DestroyedEffects, ref wearNTear.m_destroyedEffect);
                UpdateEffectList(piece.HitEffects, ref wearNTear.m_hitEffect);
                UpdateEffectList(piece.SwitchEffects, ref wearNTear.m_switchEffect);
            }

            if (__instance.TryGetComponent(out Container container))
            {
                UpdateEffectList(piece.OpenEffects, ref container.m_openEffects);
                UpdateEffectList(piece.CloseEffects, ref container.m_closeEffects);
            }

            if (!piece.CloneDoorEffectsFrom.IsNullOrWhiteSpace() &&
                ZNetScene.instance.GetPrefab(piece.CloneDoorEffectsFrom) is { } originalDoor &&
                originalDoor.TryGetComponent(out Door door))
            {
                if (__instance.TryGetComponent(out Door doorComponent))
                {
                    doorComponent.m_openEffects = door.m_openEffects;
                    doorComponent.m_closeEffects = door.m_closeEffects;
                    doorComponent.m_lockedEffects = door.m_lockedEffects;
                    
                    if (!piece.KeyItem.IsNullOrWhiteSpace() && ObjectDB.instance.GetItemPrefab(piece.KeyItem) is { } key && key.TryGetComponent(out ItemDrop keyComponent))
                    {
                        doorComponent.m_keyItem = keyComponent;
                    }
                }

                if (__instance.TryGetComponent(out DoubleDoor doubleDoor))
                {
                    doubleDoor.m_openEffects = door.m_openEffects;
                    doubleDoor.m_closeEffects = door.m_closeEffects;
                    doubleDoor.m_lockedEffects = door.m_lockedEffects;
                    
                    if (!piece.KeyItem.IsNullOrWhiteSpace() && ObjectDB.instance.GetItemPrefab(piece.KeyItem) is { } key && key.TryGetComponent(out ItemDrop keyComponent))
                    {
                        doubleDoor.m_keyItem = keyComponent;
                    }
                }
            }
            
            if (ZNetScene.instance && !piece.CloneBossStoneEffectsFrom.IsNullOrWhiteSpace() &&
                __instance.TryGetComponent(out BossStone bossStone) && bossStone.m_itemStand is {} bossItemStand)
            {
                if (bossStone.m_activateStep1.m_effectPrefabs.Length > 0) return;
                if (ZNetScene.instance.GetPrefab(piece.CloneBossStoneEffectsFrom) is { } originalBossStone &&
                    originalBossStone.TryGetComponent(out BossStone originalBossStoneComponent))
                {
                    ...
                    if (originalBossStoneComponent.m_itemStand is { } originalBossItemStand)
                    {
                        ...
                        if (!piece.BossTrophy.IsNullOrWhiteSpace() && ObjectDB.instance.GetItemPrefab(piece.BossTrophy) is { } trophy && trophy.TryGetComponent(out ItemDrop trophyComponent))
                        {
                            bossItemStand.m_supportedItems.Add(trophyComponent);
                            if (piece.GuardianPower is { } GP)
                            {
    ...
    public void UpdateEffects()
    {
        ...
        if (ZNetScene.instance && !CloneDoorEffectsFrom.IsNullOrWhiteSpace() && Prefab.TryGetComponent(out DoubleDoor doubleDoor))
        {
            var original = ZNetScene.instance.GetPrefab(CloneDoorEffectsFrom);
            if (original && original.TryGetComponent(out Door originalDoor))
            {
                doubleDoor.m_openEffects = originalDoor.m_openEffects;
                doubleDoor.m_closeEffects = originalDoor.m_closeEffects;
                doubleDoor.m_lockedEffects = originalDoor.m_lockedEffects;
            }
